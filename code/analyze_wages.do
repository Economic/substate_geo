...
use if year == 2017 using ${output}acs_wages_imputed, clear
gen new_race = .
* White
...
foreach var in hrwage0 hrwage1 hrwage2 {
	use `acs', clear
	binipolate `var' [pw=perwt], binsize(0.25) p(5(5)95) collapsefun(gcollapse)
	keep p*binned
	gen outcome = "`var'"
	tempfile acsall`var'
	save `acsall`var''
	...
	use `acs', clear
	binipolate `var' [pw=perwt], binsize(0.25) p(5(5)95) by(female) collapsefun(gcollapse)
	keep female p*binned
	gen outcome = "`var'"
	...
	use `acs', clear
	binipolate `var' [pw=perwt], binsize(0.25) p(5(5)95) by(new_race) collapsefun(gcollapse)
	keep new_race p*binned
	gen outcome = "`var'"
