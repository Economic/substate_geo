...
* calculate adjusted weeks worked
...
* simple midpoint of bin range
gen adj_wkswork0 = .
foreavlues j = 1/6 {
	replace adj_wkswork0 = 0.5 * (`bin`j'min' + `bin`j'max')
}
...
forvalues i = 0 / 1 {
	forvalues j = 1/6 {
		estimates restore female`i'_`j'
		predict xb if female == `i' & wkswork2 == `j', xb
		replace adj_wkswork1 = xb if female == `i' & wkswork2 == `j'
		drop xb
	}
}
gen adj_wkswork2 = adj_wkswork1
forvalues j = 1/6 {
	replace adj_wkswork2 = `bin`j'min' if wkswork2 == `j' & adj_wkswork1 < `bin`j'min'
	replace adj_wkswork2 = `bin`j'max' if wkswork2 == `j' & adj_wkswork1 > `bin`j'max'
}
