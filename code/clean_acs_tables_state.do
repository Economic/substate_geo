import delim ${acsdata}ACS_17_5YR_B23001_sex_age_state.csv, varnames(1) rowrange(3) clear
destring hd01*, replace
destring geoid2, replace
rename geoid2 statefips
...
foreach race in hispanic white black {
	import delim ${acsdata}ACS_17_5YR_C23002_`race'_state.csv, varnames(1) rowrange(3) clear
	destring hd01*, replace
	destring geoid2, replace
	rename geoid2 statefips
...
import delim ${acsdata}ACS_17_5YR_B23006_educ_state.csv, varnames(1) rowrange(3) clear
destring hd01*, replace
destring geoid2, replace
rename geoid2 statefips
...
use `age', clear
foreach race in hispanic white black {
	merge 1:1 statefips using ``race'', assert(3) nogenerate
}
merge 1:1 statefips using `educ', assert(3) nogenerate
